...
export class AuditService {
  ...
  private readonly auditEndpoint = "https://registry.npmjs.org/-/npm/v1/security/advisories/bulk";

  constructor(cacheDurationMs: number = 30 * 60 * 1000) { // 30 minutes default
  ...
  private filterVulnerabilitiesForVersion(
    ...
    return vulnerabilities.filter(vuln => {
      try {
        ...
        const isVulnerable = semver.satisfies(version, vuln.vulnerable_versions);
        return isVulnerable;
      } catch (error) {
        // If we can't parse the range, be conservative and include the vulnerability
        console.warn(`Could not parse vulnerable_versions range "${vuln.vulnerable_versions}" for ${vuln.title}:`, error);
        return true;
      }
    });
  }
   ...
   */
  getHighestSeverity(vulnerabilities: Vulnerability[]): "critical" | "high" | "moderate" | "low" | "info" | null {
